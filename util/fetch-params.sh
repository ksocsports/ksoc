...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/KSOCParams"
    else
        PARAMS_DIR="$HOME/.ksoc-params"
    fi
fi
...
Failed to fetch the KSOC zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
KSOC - fetch-params.sh

This script will fetch the KSOC zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common KSOC zkSNARK parameters. Note that it is
