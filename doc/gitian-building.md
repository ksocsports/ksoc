...
*Setup instructions for a gitian build of KSOC Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the KSOC
...
to the KSOC GitHub release page.
...
- [Building KSOC Core](#building-ksoc-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/ksoc-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/ksoc-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/ksoc-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/ksoc-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/ksoc-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/ksoc-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/ksoc-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@ksoc.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/ksoc-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/ksoc-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/ksoc-osx-*-build.assert
```
