...
- [Development guidelines](#development-guidelines)
    - [General KSOC Core](#general-ksoc-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
KSOC Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
KSOC Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/ksoc-project/ksoc/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_ksoc
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_ksoc --log_level=test_suite
$ valgrind -v --leak-check=full src/ksocd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_ksoc.coverage/index.html`.
```
...
However, in open source software such as KSOC Core, where everyone uses
...
pay attention to for reviewers of KSOC Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: KSOC Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against KSOC Core so that they can be integrated
...
file descriptors it will cause problems with KSOC's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:KSOC-Project/KSOC.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `ksoc-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `ksoc-cli` and the GUI debug console use this table to determine how to
