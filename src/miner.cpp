...
bool CreateCoinbaseTx(CBlock* pblock, const CScript& scriptPubKeyIn, CBlockIndex* pindexPrev)
{
    ...
    FillBlockPayee(txNew, 0, pblock->IsProofOfStake());
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("KSOCMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, nullptr, pblock, nullptr, g_connman.get())) {
        return error("KSOCMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("KSOCMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("ksoc-miner");
    const Consensus::Params& consensus = Params().GetConsensus();
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running KSOCMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("KSOCMiner exception");
    } catch (...) {
        LogPrintf("KSOCMiner exception");
    }

    LogPrintf("KSOCMiner exiting\n");
}
