...
#include "qt/ksoc/splash.h"
#include "qt/ksoc/welcomecontentwidget.h"
...
public:
    ...
    /// Get window identifier of QMainWindow (KSOCGUI)
...
{
    window = new KSOCGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &KSOCGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &KSOCGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(KSOCGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(KSOCGUI::DEFAULT_WALLET);
        ...
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &KSOCGUI::handlePaymentRequest);
        connect(window, &KSOCGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. KSOC can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    /// 6. Determine availability of data directory and parse ksoc.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("KSOC Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("KSOC Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("KSOC Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
