...
#include "qt/ksoc/forms/ui_snackbar.h"
#include "qt/ksoc/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(KSOCGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &KSOCGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
